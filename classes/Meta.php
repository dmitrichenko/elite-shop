...
class MetaCore extends ObjectModel
{
    ...
    public static function getProductMetas($id_product, $id_lang, $page_name)
    {
        ...
        if ($row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql)) {
            ...
            $sql_cat = 'SELECT id_category_default FROM `' . _DB_PREFIX_ . 'product` WHERE id_product = ' . (int)$id_product . '';
            $row_cat = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql_cat);
            $cat = new Category($row_cat['id_category_default'], (int)$id_lang);

            if(empty($row['meta_title'])) $row['meta_title'] = $row['name'];
            $row['meta_title'] .= ' - ';
            if(empty($cat->meta_title)) $row['meta_title'] .= $cat->name;
            else $row['meta_title'] .= $cat->meta_title;

            if (empty($row['meta_description'])) $row['meta_description'] = strip_tags($row['description_short']);
